Feature: Account
  ...
  Scenario: Create a new account with the same key pair twice
    ...
    And I register a new ACME account with email test@hanzo.ai and EAB key id "{acme_profile.eab_kid}" with secret "{acme_profile.eab_secret}" as acme_account2
  ...
  Scenario: Find an existing account
    ...
    And I find the existing ACME account with email test@hanzo.ai and EAB key id "{acme_profile.eab_kid}" with secret "{acme_profile.eab_secret}" as retrieved_account
  ...
  Scenario: Create a new account without EAB
    Given I have an ACME cert profile as "acme_profile"
    When I have an ACME client connecting to "{BASE_URL}/api/v1/cert-manager/acme/profiles/{acme_profile.id}/directory"
    Then I register a new ACME account with email test@hanzo.ai without EAB
