...
func TestSecretRoundTrip_TenantAlias(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", srv.URL+"/v1/kms/tenants/hanzo/secrets", bytes.NewReader(body))
	req.Header.Set("Authorization", "Bearer "+tok)
	...
	req, _ = http.NewRequest("GET",
		srv.URL+"/v1/kms/orgs/hanzo/secrets/ats/dev/sendgrid?env=dev", nil)
	req.Header.Set("Authorization", "Bearer "+tok)
	resp, _ = http.DefaultClient.Do(req)
	if resp.StatusCode != 200 {
...
func TestUnauthorized(t *testing.T) {
	...
	for _, p := range []string{
		...
		"/v1/kms/tenants/hanzo/secrets/foo/bar",
	} {
