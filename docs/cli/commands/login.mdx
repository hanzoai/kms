...
<Steps>
  <Step title="Obtain an access token">
        ...
        Now that we've set the `KMS_TOKEN` environment variable, we can use the CLI to interact with Hanzo KMS. The CLI will automatically check for the presence of the `KMS_TOKEN` environment variable and use it for authentication.
