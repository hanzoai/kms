...
<Tabs>
  <Tab title="Scanning files, directories and or git history">
	...
	When you run `hanzo-kms scan` on a Git repository, Hanzo KMS will parses the output of a `git log -p` command. This command generates [patches](https://stackoverflow.com/questions/8279602/what-is-a-patch-in-git-version-control) that Hanzo KMS uses to identify secrets in your code.
	...
	For instance, to instruct Hanzo KMS to scan a specific range of commits, use the following command: `hanzo-kms scan --log-opts="--all commitA..commitB"`. For more details, refer to the [Git log documentation](https://git-scm.com/docs/git-log).
...
A baseline allows Hanzo KMS to ignore any old findings that are already present in the baseline findings. You can create a hanzo-kms scan report by running `hanzo-kms scan` with the `--report-path` flag.
...
If you're intentionally committing a test secret that `hanzo-kms scan` might flag, you can instruct Hanzo KMS to overlook that secret with the methods listed below.
