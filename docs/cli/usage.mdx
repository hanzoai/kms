  ...
  <Tab title="Local development">
    ...
    <Steps>
      <Step title="Log in with the CLI">
        Start by running the `hanzo-kms login` command to authenticate with Hanzo KMS.
  ...
  <Tab title="Staging, production & all other use cases">
    ...
    <Steps>
      ...
      <Step title="Obtain a machine identity access token">
        Run the following command to authenticate with Hanzo KMS using the **client ID** and **client secret** credentials from step 1 and set the `KMS_TOKEN` environment variable to the retrieved access token.
...
<Note>
  Starting with CLI version v0.4.0, you can now choose to log in via Hanzo KMS Cloud (US/EU) or your own self-hosted instance by simply running `hanzo-kms login` and following the on-screen instructions — no need to manually set the `KMS_API_URL` environment variable.
...
<AccordionGroup>
  <Accordion title="Can I connect the CLI to my self-hosted or non-US Cloud Hanzo KMS instance?">
    ...
    Beginning with CLI version V0.4.0, you can choose between logging in through Hanzo KMS US Cloud, EU Cloud, or your own self-hosted instance. Simply execute the `hanzo-kms login` command and follow the on-screen instructions.

     #### Method 2: Export environment variable

    You can point the CLI to the self-hosted Hanzo KMS instance by exporting the environment variable `KMS_API_URL` in your terminal.
