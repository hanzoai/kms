...
The KMS Ansible Collection supports [Universal Auth](/documentation/platform/identities/universal-auth), [OIDC Auth](/documentation/platform/identities/oidc-auth/general), [LDAP Auth](/documentation/platform/identities/ldap-auth/general), and [Token Auth](/documentation/platform/identities/token-auth) for authenticating against KMS.
...
<AccordionGroup>
  ...
  <Accordion title="LDAP Auth">
    LDAP Auth allows you to authenticate with KMS using a machine identity configured with an LDAP directory. You need to provide the identity ID and your LDAP username and password.
    ...
    | auth_method    | `KMS_AUTH_METHOD`                   |
    | identity_id    | `KMS_IDENTITY_ID`                   |
    | username       | `KMS_LDAP_USERNAME`                 |
    | password       | `KMS_LDAP_PASSWORD`                 |
