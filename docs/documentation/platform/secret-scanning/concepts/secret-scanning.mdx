...
In addition to real-time monitoring, Hanzo KMS supports both full repository scans and lightweight diff scans, as well as local pre-commit scanning via the [KMS CLI](/cli/commands/scan). This allows teams to prevent secret leaks before they ever reach production.
