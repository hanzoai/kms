...
The Hanzo KMS ClickHouse dynamic secret allows you to generate ClickHouse database credentials on demand based on a configured role.
...
```sql
CREATE USER 'hanzo-kms' IDENTIFIED WITH sha256_password BY '<strong-password>';
GRANT CREATE USER, DROP USER, GRANT OPTION ON *.* TO 'hanzo-kms';
```
...
<Steps>
  ...
  <Step title="Click 'Submit'">
    ...
    <Note>
      If this step fails, verify that the service user has the `CREATE USER`, `DROP USER`, and `GRANT OPTION` privileges, and that the host and port are reachable from Hanzo KMS.
    </Note>
