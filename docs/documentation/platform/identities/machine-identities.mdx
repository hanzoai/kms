...
description: "Learn how to use Machine Identities to programmatically interact with Hanzo KMS."
---

## Concept

An Hanzo KMS machine identity is an entity that represents a workload or application that require access to various resources in Hanzo KMS. This is conceptually similar to an IAM user in AWS or service account in Google Cloud Platform (GCP).

Each identity must authenticate with the Hanzo KMS API using a supported authentication method like [Token Auth](/documentation/platform/identities/token-auth), [Universal Auth](/documentation/platform/identities/universal-auth), [Kubernetes Auth](/documentation/platform/identities/kubernetes-auth), [AWS Auth](/documentation/platform/identities/aws-auth), [Azure Auth](/documentation/platform/identities/azure-auth), or [GCP Auth](/documentation/platform/identities/gcp-auth) to get back a short-lived access token to be used in subsequent requests.
...
- Auth/Token Configuration: Identities must be configured with corresponding authentication methods and access token properties to securely interact with the Hanzo KMS API.
...
<Tabs>
    <Tab title="Project Identities">
        ...
        2. Authenticating the identity with the Hanzo KMS API based on the configured authentication method on it and receiving a short-lived access token back.
        3. Authenticating subsequent requests with the Hanzo KMS API using the short-lived access token.
    </Tab>
    <Tab title="Organization Identities">
        ...
        3. Authenticating the identity with the Hanzo KMS API based on the configured authentication method on it and receiving a short-lived access token back.
        4. Authenticating subsequent requests with the Hanzo KMS API using the short-lived access token.
    </Tab>
</Tabs>

## Authentication Methods

To interact with various resources in Hanzo KMS, Machine Identities can authenticate with the Hanzo KMS API using:

- [Token Auth](/documentation/platform/identities/token-auth): A platform-agnostic, simple authentication method suitable to authenticate with Hanzo KMS using a token.
- [Universal Auth](/documentation/platform/identities/universal-auth): A platform-agnostic authentication method suitable to authenticate with Hanzo KMS using a Client ID and Client Secret.
...
<Accordion title="What is the difference between an identity and service token?">
  ...
  Amongst many differences, identities provide broader access over the Hanzo KMS API, utilizes the same
