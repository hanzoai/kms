...
sequenceDiagram
  ...
  Client->>KMS: Send signed JWT to /api/v1/auth/oidc-auth/login

  Note over KMS,Idp: Step 3: Query verification
  KMS->>Idp: Request JWT public key using OIDC Discovery
  Idp-->>KMS: Return public key

  Note over KMS: Step 4: JWT validation
...
<Steps>
    ...
    <Step title="Accessing the Hanzo KMS API with the identity">
        ...
        script:
            ...
            - export KMS_TOKEN=$(hanzo-kms login --method=oidc-auth --machine-identity-id=4e807a78-1b1c-4bd6-9609-ef2b0cf4fd54 --oidc-jwt=$KMS_ID_TOKEN --silent --plain)
