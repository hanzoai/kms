...
description: "Learn more about integrating with Hanzo KMS KMS using KMIP (Key Management Interoperability Protocol)."
---
...
Hanzo KMS KMS provides Key Management Interoperability Protocol (KMIP) support for integration with KMIP-compatible clients. This allows for enhanced key management across various applications that support the KMIP 1.4 protocol.
...
2. The KMIP server acts as a proxy and forwards requests to Hanzo KMS KMS
...
- **KMIP Server → Hanzo KMS Platform**: The KMIP server needs outbound HTTP access to Hanzo KMS. For self-hosted instances, ensure connectivity to your custom domain.
...
<Steps>
  <Step title="Enable KMIP at the Organization Level">
    First, you need to enable KMIP for your entire Hanzo KMS organization and set up its PKI infrastructure.
    ...
    <Info>
    You do not need to manage these certificates yourself; Hanzo KMS handles the PKI infrastructure for you.
    </Info>
  </Step>
  ...
  <Step title="Create a Machine Identity for the KMIP Server">
    ...
    This machine identity will be used by the KMIP server to authenticate and forward client requests to Hanzo KMS KMS.
  </Step>
  ...
  <Step title="Deploy the KMIP Server">
    ...
    If you need to install specific versions of the CLI, you can find them on the [KMS CLI GitHub Releases](https://github.com/Hanzo KMS/cli/releases).
  ...
  <Step title="Generate Client Certificate">
    ...
    Configure your KMIP-compatible applications or tools to use these credentials when connecting to the KMIP server. The client will now authenticate via mTLS and perform authorized key management operations through the KMIP server, which proxies requests to Hanzo KMS KMS.
  </Step>
</Steps>
...
2. **Client Certificates**: The certificate credentials generated from your Hanzo KMS KMS project:
...
<Steps>
  ...
  <Step title="Prepare Certificate Files">
    Organize the certificate materials you downloaded when generating the client certificate from the Hanzo KMS KMS project. You should have three files:
  ...
  <Step title="Test the Connection">
    ...
    If the connection is successful, your KMIP client is now integrated with Hanzo KMS KMS and can perform key management operations according to the permissions you assigned.
