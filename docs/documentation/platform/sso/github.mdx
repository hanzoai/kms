...
<Steps>
  ...
  <Step title="Add your OAuth application credentials to Hanzo KMS">
    ...
    - `SITE_URL`: The URL of your self-hosted instance of Hanzo KMS - should be an absolute URL including the protocol (e.g. https://kms.hanzo.ai)
...
<AccordionGroup>
  <Accordion title="Why is GitHub SSO not working?">
    ...
    For example, if the former is `https://kms.hanzo.ai/api/v1/sso/github` then the latter should be `https://kms.hanzo.ai`.
  </Accordion>
</AccordionGroup>