...
description: "Learn how to sync OIDC group members to matching groups in Hanzo KMS."
---
...
<Warning>
    Group membership changes in your OIDC provider only sync with Hanzo KMS when a
    user logs in via OIDC. For example, if you remove a user from a group in your OIDC provider,
    this change will not be reflected in Hanzo KMS until their next OIDC login.
    To ensure this behavior, Hanzo KMS recommends enabling Enforce OIDC SSO in the OIDC settings.
</Warning>
