...
<Tabs>
  ...
  <Steps>
    <Step title="Create the Managing User IAM Role">
      ...
      4. Optionally, enable **Require external ID** and enter your Hanzo KMS **project ID** to further enhance security.
    </Step>
...
<Steps>
  ...
  <Step title="Provide the inputs for AWS KMS">
    ...
    <ParamField path="AWS KMS Key ID" type="string">
    	Key ID of the AWS KMS Key. If left blank, Hanzo KMS will generate and use a new AWS KMS Key in the specified region.
