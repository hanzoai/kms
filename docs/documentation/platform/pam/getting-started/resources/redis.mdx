...
title: "Redis"
sidebarTitle: "Redis"
description: "Learn how to configure Redis access through Hanzo KMS PAM for secure, audited, and just-in-time access to your Redis instances."
---

Hanzo KMS PAM supports secure, just-in-time access to Redis instances.
...
Redis server forks and variants based on the RESP protocol such as [Valkey](https://valkey.io) are also supported by Hanzo KMS PAM.

## How It Works

Redis access in Hanzo KMS PAM uses an KMS Gateway to securely proxy connections to your Redis server. When a user requests access, Hanzo KMS establishes a secure tunnel through the Gateway, enabling secure access without exposing your Redis instance directly.
...
Before configuring Redis access in Hanzo KMS PAM, you need:
