...
description: "Learn how to configure Windows Server access through Hanzo KMS PAM for secure, audited, and just-in-time access to your Windows servers via RDP."
---

Hanzo KMS PAM supports secure, just-in-time access to Windows servers via the Remote Desktop Protocol (RDP).
...
Windows Server access in Hanzo KMS PAM uses an KMS Gateway to securely proxy RDP connections to your Windows server. When a user requests access, Hanzo KMS establishes a secure tunnel through the Gateway, enabling access without exposing your server directly.
...
Before configuring Windows Server access in Hanzo KMS PAM, you need:
