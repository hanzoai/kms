...
<Warning>
  **Gateway Required**: Active Directory access requires an KMS Gateway to be deployed and registered with your KMS instance. The Gateway must have network connectivity to your domain controller on the LDAP port (default: 389).
</Warning>
...
<Steps>
  <Step title="Ensure Gateway is Running">
    Before creating the resource, ensure you have an KMS Gateway running and registered with your KMS instance. The Gateway must have network access to your AD domain controller.
  </Step>
  ...
  <Step title="Create the Resource in KMS">
    ...
    Clicking **Create Resource** will trigger a connection test from the selected Gateway to your domain controller. KMS verifies that the server is reachable and responds to LDAP requests.
  </Step>
</Steps>
