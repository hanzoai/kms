...
At its core, Hanzo KMS PAM is designed to decouple **user identity** from **infrastructure credentials**. Instead of sharing static passwords or SSH keys, users authenticate with their SSO identity, and Hanzo KMS handles the rest.
...
To successfully implement Hanzo KMS PAM, it is essential to understand the relationship between the following components:
...
Hanzo KMS PAM uses a secure proxy-based architecture to connect users to resources without direct network exposure.
