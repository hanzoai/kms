...
<AccordionGroup>
    <Accordion title="Is this a real SSH session?">
        Yes. Unlike the PostgreSQL web access terminal which is a custom SQL REPL, the SSH terminal is a real SSH shell session proxied through the Hanzo KMS Gateway. You get a full interactive shell with all the capabilities of a native SSH connection.
    </Accordion>
</AccordionGroup>
