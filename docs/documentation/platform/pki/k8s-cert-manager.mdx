...
<Steps>
  ...
  <Step title="Create the cert-manager Issuer connecting to Hanzo KMS ACME server">
    ...
    This resource configures cert-manager to use your Hanzo KMS PKI collection's ACME server for certificate issuance.
...
<Accordion title="What fields can be configured on the Certificate resource?">
    ...
    <Note>
        Currently, not all fields are supported by the Hanzo KMS PKI ACME server.
    </Note>
