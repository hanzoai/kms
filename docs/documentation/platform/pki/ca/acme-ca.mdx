...
CNAME delegation is an industry-standard pattern that lets you redirect ACME DNS-01 challenge records to a separate DNS zone. Instead of granting KMS write access to your main domain zone, you point `_acme-challenge` via a CNAME to a dedicated zone where KMS creates the TXT records.

### How It Works

If you have a CNAME record pointing `_acme-challenge.example.com` to another domain (e.g., `_acme-challenge.ssl.example.com`), you configure KMS to manage the target zone (e.g., `ssl.example.com`). KMS creates the TXT record in that target zone. During validation, the ACME CA queries `_acme-challenge.example.com`, follows the CNAME, and finds the TXT record at the delegated location.

To use CNAME delegation, you need to:

1. **Create a CNAME record** in your domain's DNS that points `_acme-challenge.{your-domain}` to a target in the zone configured with KMS. For subdomains, create additional CNAMEs as needed (e.g., `_acme-challenge.app.example.com` CNAME to `_acme-challenge.ssl.example.com`).

2. **Register the ACME CA** in KMS with the **hosted zone ID** of the target zone (e.g., `ssl.example.com` if your CNAME points to `_acme-challenge.ssl.example.com`).

KMS will automatically resolve the CNAME, create the TXT record at the target, wait for DNS propagation, and clean up after validation completes.

### Example: Delegating Across DNS Providers

Suppose your primary domain `example.com` is managed in Cloudflare, but you want KMS to perform ACME challenges through a Route53 zone. This is useful when your main DNS provider isn't supported by KMS, or when you want to keep your production DNS provider credentials separate from certificate automation.

1. **Create a dedicated Route53 hosted zone** for `acme.example.com`, KMS needs write permissions here.
2. **Add a CNAME record in Cloudflare** pointing `_acme-challenge.example.com` to `_acme-challenge.acme.example.com`.
3. **Create an AWS App Connection** in KMS with Route53 permissions for the `acme.example.com` zone.
4. **Register your ACME CA in KMS** using the Route53 hosted zone ID for `acme.example.com`.

When KMS issues a certificate for `example.com`:
- KMS writes the TXT record to `_acme-challenge.acme.example.com` in Route53.
- The ACME CA queries `_acme-challenge.example.com` in Cloudflare, follows the CNAME to Route53, and finds the TXT record.
- After validation, KMS cleans up the TXT record in Route53.

This way, KMS never needs access to your Cloudflare account, it only writes to the dedicated Route53 zone used for ACME challenges.

### Common Use Cases

- **Least-privilege access**: Grant KMS write access only to a dedicated subdomain zone instead of your entire domain zone.
- **Cross-provider setups**: Keep your domain DNS on one provider while using a different, KMS-supported provider for ACME challenges.
...
<AccordionGroup>
  <Accordion title="What DNS validation methods are supported?">
    Currently, KMS supports DNS-01 validation through AWS Route53, Cloudflare, Azure DNS, and DNS Made Easy. The DNS-01 challenge method is preferred for ACME integrations because it:
  ...
  <Accordion title="Does KMS support DNS CNAME delegation for ACME challenges?">
    Yes. You can delegate `_acme-challenge` records to a different DNS zone by setting up a CNAME, then configuring KMS to write TXT records in the target zone. See the [DNS CNAME Delegation](#dns-cname-delegation) section above for setup instructions.
  </Accordion>
</AccordionGroup>
