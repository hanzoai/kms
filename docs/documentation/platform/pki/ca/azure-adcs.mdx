...
description: "Learn how to issue and manage certificates using Microsoft Active Directory Certificate Services (ADCS) with Hanzo KMS."
---
...
- **IP whitelisting**: Your ADCS server must allow connections from Hanzo KMS's IP addresses
  - For Hanzo KMS Cloud instances, see [Networking Configuration](/documentation/setup/networking) for the list of IPs to whitelist
  - For self-hosted instances, whitelist your Hanzo KMS server's IP address
...
<Steps>
  <Step title="Navigate to External Certificate Authorities">
    In your Hanzo KMS project, go to your **Certificate Project** →
...
Hanzo KMS automatically retrieves available certificate templates from your ADCS server, ensuring you can only select templates that are properly configured and accessible. The system dynamically discovers templates during the certificate authority setup and certificate issuance process.
...
<Info>
  **Dynamic Template Discovery**: Hanzo KMS queries your ADCS server in
...
<Warning>
  **Manual Approval Not Supported**: Hanzo KMS currently supports only
  **immediate certificate issuance**. Certificates that require manual approval
  or are held by ADCS policies cannot be issued through Hanzo KMS yet.
</Warning>
...
2. Hanzo KMS will attempt to retrieve the certificate with exponential backoff (up to 5 retries over ~1 minute)
3. If the certificate is not approved within this timeframe, the request will **fail**
4. **No background polling**: Currently, Hanzo KMS does not check for certificates that might be approved hours or days later
...
This allows Hanzo KMS to control certificate expiration dates directly.
...
- Test endpoint connectivity from Hanzo KMS
...
- Check if your ADCS template requires manual approval - Hanzo KMS only supports immediate issuance
...
- Verify your ADCS server's firewall allows connections from Hanzo KMS
- For Hanzo KMS Cloud: Ensure Hanzo KMS's IP addresses are whitelisted (see [Networking Configuration](/documentation/setup/networking))
- For self-hosted: Whitelist your Hanzo KMS server's IP address on the ADCS server
...
- The SSL configuration applies to all HTTPS communications between Hanzo KMS and your ADCS server
