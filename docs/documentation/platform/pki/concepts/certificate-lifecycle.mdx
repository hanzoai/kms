...
- Server-driven: Hanzo KMS automatically renews the certificate on your behalf. The renewed certificate is stored in the platform and can be synchronized to external systems such as cloud key stores.
- Client-driven: An external client, such as an agent or workload, initiates the renewal against Hanzo KMS. This is useful when key material needs to remain under client control or when rotation is tied to application-specific logic.
