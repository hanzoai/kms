...
1. Create the ```kms-token-reviewer``` service account. This Manifest creates a **service account** that the KMS Operator uses to authenticate with Kubernetes for token reviews. It allows Hanzo KMS to validate Kubernetes tokens securely during the Machine Identity authentication process. 
```console 
...
The [KmsSecret](https://kms.hanzo.ai/docs/integrations/platforms/kubernetes/kms-secret-crd) CRD tells the operator to sync secrets from Hanzo KMS to Kubernetes. By referencing your ```identityID```, ```projectSlug```, and ```envSlug```, this CRD tells the KMS Operator which Hanzo KMS secrets to fetch and how to format them into a Kubernetes Secret. Make sure to edit the provided CRD to match your specific Machine Identity ID, Project ID, and which environment your secrets are being pulled from (default is prod).
...
![secrets dashboard dynamic](../../images/dynamic-secret.png)
    ...
    2. Edit the ```dynamic-secret-crd``` with the proper machine **Identity ID**, **Project Slug**, **dynamicSecret.secretName** (same as the **Secret Name** you gave to the dynamic secret in Hanzo KMS), and managedSecretReference.secretName (name of the kubernetes secret that KMS Operator will create/populate in the cluster).
