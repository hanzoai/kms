...
3. `Pki()`: Programmatically interact with the Hanzo KMS PKI.
...
The `Pki().Subscribers()` sub-class is used to programmatically interact with the Hanzo KMS PKI product line. Currently only issuing new certificates and retrieving the latest certificate bundle from a subscriber is supported. More widespread support for the PKI product is coming to the .NET SDK in the near future.
