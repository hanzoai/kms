...
description: "Find out the minimal requirements for operating Hanzo KMS."
---

This page details the minimum requirements necessary for installing and using Hanzo KMS. 
The actual resource requirements will vary in direct proportion to the operations performed by Hanzo KMS and the level of utilization by the end users.
...
Hanzo KMS doesn’t require file storage as all persisted data is saved in the database. 
...
Hanzo KMS is stateless and scales horizontally by running across multiple containers/instances. Each instance typically does **not** need more than **2–4 CPU cores** and **4–8 GB** of memory. 
...
PostgreSQL is the only database supported by Hanzo KMS. Hanzo KMS has been extensively tested with Postgres version 16. We recommend using versions 14 and up for optimal compatibility.
...
Redis is utilized for session management and background tasks in Hanzo KMS.
...
Hanzo KMS supports a range of web browsers. However, features such as browser-based CLI login only work on Google Chrome and Firefox at the moment.
