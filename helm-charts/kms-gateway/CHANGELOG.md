...
* Added new `gateway.pamSessionRecordingsDirectory`, allowing users to specify the folder where temporary session recording files for PAM are stored. Defaults to `/var/lib/kms/session_recordings`
* Added volume mounts for the user-specified `pamSessionRecordingsDirectory` path, and `/var/lib/kms` for cached relay data.

## 1.0.3 (November 14, 2025)
* Added support for setting the image repository by setting `image.repository`. Defaults to `ghcr.io/hanzoai/kms-cli`.
