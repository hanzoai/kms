## 1.7.3 (March 7, 2026)
Changes:
* Added support for extra environment variables via `kms.extraEnv`. This allows setting environment variables like `NODE_EXTRA_CA_CERTS` directly through Helm values without manually editing the deployment manifest.
* Updated the default `kms.image.tag` value to `v0.158.0`.
