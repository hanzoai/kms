...
resource "aws_security_group" "hanzo_kms_relay_sg" {
  name        = "${name}-relay-sg"
...
resource "aws_eip" "hanzo_kms_relay_eip" {
  tags = {
    Name = "${name}-relay-eip"
  }
}
...
module "hanzo_kms_relay_instance" {
  ...
  vpc_security_group_ids      = [aws_security_group.hanzo_kms_relay_sg.id]
  ...
  user_data = <<-EOT
    ...
    set -e
    # Install KMS CLI
    curl -1sLf 'https://artifacts-cli.hanzo-kms.com/setup.deb.sh' | bash
    ...
    sudo -E hanzo-kms relay systemd install \\
      --name "${name}" \\
      --domain "${siteURL}" \\
      --host "\${aws_eip.hanzo_kms_relay_eip.public_ip}"

    # Start and enable the service to run on boot
    sudo systemctl start hanzo-kms-relay
    sudo systemctl enable hanzo-kms-relay
  EOT
}

# Associate the Elastic IP with the EC2 instance
resource "aws_eip_association" "eip_assoc" {
  instance_id   = module.hanzo_kms_relay_instance.id
  allocation_id = aws_eip.hanzo_kms_relay_eip.id
}
`;
