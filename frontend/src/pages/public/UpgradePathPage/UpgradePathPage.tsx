                  ...
                  const versionsWithDbMigrations = upgradeResult.path
                    .filter((step, index) => {
                      ...
                      const possibleKeys = [
                        ...
                        step.version.replace(/^hanzo-kms\/v?/, ""),
                        step.version.replace(/^hanzo-kms\/v?/, "").replace(/-[a-zA-Z]+$/, "")
                      ];
                  ...
                  <div className="space-y-6">
                    {(() => {
                          ...
                          (bc) =>
                            !upgradeResult.path.some((step) => {
                              const normalizeVersion = (v: string) =>
                                v.replace(/^(hanzo-kms\/)?v?/, "").replace(/-[a-zA-Z]+$/, "");
                              return (
                      ...
                      const versionChanges = upgradeResult.breakingChanges.find((bc) => {
                        ...
                        const normalizeVersion = (v: string) => {
                          return v.replace(/^(hanzo-kms\/)?v?/, "").replace(/-[a-zA-Z]+$/, "");
                        };
